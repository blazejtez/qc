...
def gradient_descent_constrained(goal_gradient, x0, lambd0, lr=0.001, tol=0.005, max_iter=1000):
    ...
    m_x = cp.zeros((x.size+lambd0.size,1),dtype=np.float32)
    v_x = cp.zeros((x.size+lambd0.size,1),dtype=np.float32)
    ...
    for i in range(max_iter):
        ...
        if grad_lambda is not None and lambd.size > 0:
            ...
            # Update lambda
            lambd_new = lambd - lr * m_hat_lambda / (cp.sqrt(v_hat_lambda) + epsilon)
        else:
            lambd_new = lambd
...
def find_lowest_eigenvalues(A, initial_x, num_eigenvalues=5, lr=1e-5, tol=1e-5, max_iter=1000):
    eigenvectors = []
    eigenvalues = []
    x = initial_x
    ...
    for i in range(num_eigenvalues):
        ...
        goal_gradient = GoalGradient(A, x, Y=Y)

        # Perform constrained gradient descent
        x, lambd = gradient_descent_constrained(goal_gradient, x, lambd0, lr=lr, tol=tol, max_iter=max_iter)
        ...
        eigenvalues_cp = cp.array(eigenvalues).reshape((-1,1))
        x = cp.concatenate((initial_x.T, eigenvalues_cp), axis=1).T
...
# eigenvalues, eigenvectors = find_lowest_eigenvalues(A, x0, num_eigenvalues=5, lr = 1e-6, tol=1e-4, max_iter = 10000)
# end_time = time.time()
#
# # Display the results
