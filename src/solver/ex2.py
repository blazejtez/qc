...
def gradient_descent_constrained(goal_gradient, x0, lambd0, lr_x=0.001, lr_lambda=0.1, tol=0.005, max_iter=1000):
    ...
    for i in range(max_iter):
        ...
        eigenvalue = goal_gradient.objective_function(x_new, lambd_new)
        '''
...
def find_lowest_eigenvalues(A, initial_x, num_eigenvalues=5, lr_x=1e-5, lr_lambda=0.1, tol=1e-5, max_iter=1000,
    ...
    for i in range(num_eigenvalues):
        ...
        lambd0 = cp.zeros((len(eigenvectors), 1), dtype=cp.float32) if eigenvectors else cp.zeros((0, 1),
                                                                                                  dtype=cp.float32)
...
goal_gradient = GoalGradient(A=A, x=v_init)
