...
def gradient_descent_constrained(goal_gradient, x0, lambd0, lr_x=0.001, lr_lambda=0.1, tol=0.005, max_iter=1000):
    ...
    for i in range(max_iter):
        ...
        if i % 100 == 1 or i == max_iter - 1:
            ...
            if lambd is not None:
                print(
                    f"{i:6d}, {eigenvalue[0, 0]:.7f}, {x_factor[0, 0]:.7f}, {lambda_factor[0, 0]:.7f}, {step_x:.7f}, {constraint_violation:.7f}")
            else:
...
def find_lowest_eigenvalues(A, initial_x, num_eigenvalues=5, lr_x=1e-5, lr_lambda=0.1, tol=1e-5, max_iter=1000,
                            initial_Y=None, initial_lambd=None):
    ...
    for i in range(num_eigenvalues):
        ...
        x, lambd = gradient_descent_constrained(goal_gradient, x0, lambd0, lr_x=lr_x, lr_lambda=lr_lambda, tol=tol,
                                                max_iter=max_iter)
...
eigenvalues, eigenvectors = find_lowest_eigenvalues(A, h_1s, num_eigenvalues=4, lr_x=1e-6, lr_lambda=1e-2,
                                                    # e-3 dla pierwszych 3 wartosci
                                                    tol=1e-10, max_iter=10000, initial_Y=None, initial_lambd=None)
end_time = time.time()
